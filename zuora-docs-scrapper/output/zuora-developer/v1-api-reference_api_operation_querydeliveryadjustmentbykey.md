---
title: "QueryDeliveryAdjustmentByKey"
url: "https://developer.zuora.com/v1-api-reference/api/operation/queryDeliveryAdjustmentByKey/"
product: "zuora-developer"
scraped_at: "2026-01-02T07:06:46.363Z"
---
...
| keyrequired | stringID or number of the delivery adjustment. |
| --- | --- |
...
| sort[] | Array of stringsA case-insensitive query parameter that specifies the sort order of the list, which can be either ascending (e.g. accountnumber.ASC) or descending (e.g. accountnumber.DESC). You cannot sort on properties in arrays. If the array-type properties are specified for the sort[] parameter, they are ignored. Supported sortable fields include:idadjustmentnumbercreditmemonumbercreditmemoiddebitmemonumbersegmentidchargeidchargenumbersubscriptionidsubscriptionnumberoriginalsubidaccountidinvoiceowneraccountid{indexedcustomfield}: Use the format like customField__c to filter on custom fields. |
| expand[] | Array of stringsAllows you to expand responses by including related object information in a single call. |
| filter[] | Array of stringsA case-insensitive query parameter that specifies the sort order of the list, which can be either ascending (e.g. accountnumber.ASC) or descending (e.g. accountnumber.DESC). You cannot sort on properties in arrays. If the array-type properties are specified for the sort[] parameter, they are ignored. Supported sortable fields include:idadjustmentnumbercreditmemonumbercreditmemoiddebitmemonumbersegmentidchargeidchargenumbersubscriptionidsubscriptionnumberoriginalsubidaccountidinvoiceowneraccountid{indexedcustomfield}: Use the format like customField__c to filter on custom fields. |
...
curl \-i \-X GET \\
  'https://rest.test.zuora.com/object-query/delivery-adjustments/{key}?pageSize=10&cursor=string&sort%5B%5D=string&expand%5B%5D=string&filter%5B%5D=string&fields%5B%5D=string&includeNullFields=false' \\
