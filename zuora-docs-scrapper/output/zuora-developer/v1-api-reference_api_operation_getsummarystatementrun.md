---
title: "GetSummaryStatementRun"
url: "https://developer.zuora.com/v1-api-reference/api/operation/getSummaryStatementRun/"
product: "zuora-developer"
scraped_at: "2026-01-02T07:10:23.634Z"
---

## Retrieve a summary statement run

Retrieves a statement run by using the statement run ID or number.
...
| keyrequired | stringStatement run ID or number. |
| --- | --- |
...
| sort[] | Array of stringsA case-insensitive query parameter that specifies the sort order of the list, which can be either ascending (e.g. accountnumber.ASC) or descending (e.g. accountnumber.DESC). You cannot sort on properties in arrays. If the array-type properties are specified for the sort[] parameter, they are ignored.Items Enum: "endDate" "startDate" "statementRunNumber" |
| expand[] | Array of stringsAllows you to expand responses by including related object information in a single call. |
| filter[] | Array of stringsA case-insensitive filter on the list.Items Enum: "accountId" "id" "startDate" "statementRunNumber" "status" |
...
get/object-query/summarystatementruns/{key}
...
curl \-i \-X GET \\
  'https://rest.test.zuora.com/object-query/summarystatementruns/{key}?pageSize=10&cursor=string&sort%5B%5D=endDate&expand%5B%5D=string&filter%5B%5D=accountId&fields%5B%5D=string&includeNullFields=false' \\
