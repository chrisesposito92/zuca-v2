---
title: "PUT CancelSubscription"
url: "https://developer.zuora.com/v1-api-reference/api/operation/PUT_CancelSubscription/"
product: "zuora-developer"
scraped_at: "2026-01-02T06:50:49.322Z"
---
...
| bookingDate | string <date>The booking date that you want to set for the amendment contract when you cancel the subscription. The default value is the current date when you make the API call. |
| cancellationEffectiveDate | string <date>Date the cancellation takes effect, in the format yyyy-mm-dd. Use only if cancellationPolicy is SpecificDate. Should not be earlier than the subscription contract-effective date, later than the subscription term-end date, or within a period for which the customer has been invoiced. |
| cancellationPolicyrequired | stringCancellation method. Possible values are: EndOfCurrentTerm, EndOfLastInvoicePeriod, SpecificDate. If using SpecificDate, the cancellationEffectiveDate field is required. |
...
| contractEffectiveDate | string <date>The date when the customer notifies you that they want to cancel their subscription. |
