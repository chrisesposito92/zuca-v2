...
export async function POST(request: NextRequest) {
  try {
    ...
    const { session_id, target_type, rating, categories, comment } = body;
    ...
    // Validate categories if provided (must be an array of strings)
    if (categories !== undefined && (!Array.isArray(categories) || !categories.every(c => typeof c === 'string'))) {
      return NextResponse.json(
        { error: 'Invalid categories - must be an array of strings' },
        { status: 400 }
      );
    }
